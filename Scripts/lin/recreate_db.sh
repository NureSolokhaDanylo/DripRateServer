...
docker compose -f "${REPO_ROOT}/compose.yaml" --env-file "${ENV_PATH}" --project-name driprateserver up --build -d
docker compose -f "${REPO_ROOT}/compose.yaml" --env-file "${ENV_PATH}" --project-name driprateserver down -v
